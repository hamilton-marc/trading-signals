...
from scripts.paths import DATA_DAILY_DIR, DATA_MONTHLY_DIR, DATA_WEEKLY_DIR, META_ERRORS_DIR, META_LATEST_DIR, META_SUMMARIES_DIR, STRATEGIES_MTF_V2_DIR
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--daily-input-dir", default=str(DATA_DAILY_DIR), help="Directory with daily OHLC CSV files")
    parser.add_argument("--weekly-input-dir", default=str(DATA_WEEKLY_DIR), help="Directory with weekly OHLC CSV files")
    parser.add_argument("--monthly-input-dir", default=str(DATA_MONTHLY_DIR), help="Directory with monthly OHLC CSV files")
    parser.add_argument(
        "--allow-derived-fallback",
        action="store_true",
        help="Allow deriving weekly/monthly bars from daily if external files are missing",
    )
    parser.add_argument("--out-dir", default=str(STRATEGIES_MTF_V2_DIR), help="Directory for per-symbol output CSVs")
    parser.add_argument(
        "--latest-file",
        default=str(META_LATEST_DIR / "mtf_entry_exit_v2_latest.csv"),
        help="CSV path for latest strategy status per symbol",
    )
    parser.add_argument(
        "--summary-file",
        default=str(META_SUMMARIES_DIR / "mtf_entry_exit_v2_summary.csv"),
        help="CSV path for summary metrics per symbol",
    )
    parser.add_argument(
        "--errors-file",
        default=str(META_ERRORS_DIR / "mtf_entry_exit_v2_errors.csv"),
